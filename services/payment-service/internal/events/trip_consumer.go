...
func (c *TripConsumer) handleTripAccepted(ctx context.Context, payload messaging.PaymentTripResponseData) error {
	log.Printf("Handling trip accepted for TripID: %s, UserID: %s, DriverID: %s, Amount: %.2f %s",
		payload.TripID, payload.UserID, payload.DriverID, payload.Amount, payload.Currency)
	...
	paymentPayload := messaging.PaymentEventSessionCreatedData{
		...
		Amount:    float64(paymentSession.Amount) / 100.0, // Convert from cent to dollars
		Currency:  paymentSession.Currency,
	}

	payloadBytes, err := json.Marshal(paymentPayload)
	if err != nil {
		log.Printf("Failed to marshal payment session created payload: %v", err)
		return err
	}
	...
	if err := c.rabbitmq.PublishMessage(ctx, contracts.PaymentEventSessionCreated,
		...
		log.Printf("Failed to publish payment session created message: %v", err)
		return err
	}
